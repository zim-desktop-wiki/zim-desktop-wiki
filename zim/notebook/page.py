...
class Page(Path, SignalEmitter):
	...
	def __init__(self, path, haschildren, file, folder):
		...
		#add some variables for merging conflicting disk and buffer contents
	...
	def _refresh_view_merge_changes(self):
		...
		self._ui_object.textview.set_property('editable',False)
	...
	def _settree_and_update_view(self, newlines):
		...
		for k in reversed(range(len(newlines))):
			...
			if delnb > 0:
				buffer.do_delete_range(buffer, start,end)
			...
			print(list(self._ui_object))
			#inserttree.resolve_images(self._ui_object.ui.notebook, self)
			inserttree.resolve_images(self._ui_object.notebook, self)

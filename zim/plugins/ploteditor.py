...
ui_actions = (
	...
	('insert_plot', None, _('_Plot...'), '', _('Insert plot'), False),
		# T: menu item for insert plot plugin
)


class InsertPlotPlugin(PluginClass):

	plugin_info = {
		'name': _('Insert Plot'), # T: plugin name
		'description': _('''\
This plugin provides a plot editor for zim based on GNU R.
'''), # T: plugin description
		'help': ':Plugins:Plot Editor',
		'author': 'Lee Braiden',
	}
	...
	def __init__(self, ui):
		...
		if self.ui.ui_type == 'gtk':
			...
			self.register_image_generator_plugin('plot')
	...
	def do_populate_popup(self, menu, buffer, iter, image):
		...
		item = gtk.MenuItem(_('_Edit plot')) # T: menu item in context menu
...
class InsertPlotDialog(ImageGeneratorDialog):
	...
	def __init__(self, ui, image=None):
		...
		ImageGeneratorDialog.__init__(self, ui, _('Insert Plot'), # T: dialog title
			generator, image, help=':Plugins:Plot Editor' )
...
class PlotGenerator(object):
	...
	def __init__(self):
		file = data_file('templates/_Plot.r')
		assert file, 'BUG: could not find templates/_Plot.r'
		self.template = GenericTemplate(file.readlines(), name=file)
		self.plotscriptfile = TmpFile('plot-script.r')

...
export PKG_CONFIG_PATH="/mingw64/lib/pkgconfig:${__old_pkg_config_path}"
__old_path="${PATH}"
export PATH="/mingw64/bin:${__old_path}"
...
[[ "${__usage+x}" ]] || read -r -d '' __usage <<-'EOF' || true # exits non-zero when EOF encountered
  -m --skip-msys   Skip MSys preparation
...
[[ "${__helptext+x}" ]] || read -r -d '' __helptext <<-'EOF' || true # exits non-zero when EOF encountered
 This is the build script to build the Windows installer for Zim.
EOF
...
[[ "${MSYSTEM_CARCH:-}" ]] || emergency "Cannot continue without MSYSTEM_CARCH. Are you running MSys? "
...
if ! [[ "${arg_m:?}" = "1" ]]; then
  ...
  info "Preparing MSys environment ..."
...
info "Checking MSys environment ..."
...
check pkg-config --print-errors --exists 'gobject-introspection-1.0 >= 1.46.0' || {
  error "GObject-Introspection not found, Please check above errors and correct them"
  exit 1
}

__venv_dir=${__dir}/build/venv
...
info "Entering virtual environment for Zim ..."
source ${__venv_dir}/bin/activate

if ! [[ "${arg_e:?}" = "1" ]]; then

  info "Initializing virtual environment for Zim ..."
  python -m pip install -U pip
  pip install PyGObject xdg pyinstaller

fi

info "Checking virtual environment for Zim ..."
check_module() {
  python3 -c "$1" 2>&- || {
...
cd ${__dir}/..
__zim_ver="$(python3 setup.py --version)"
cd ${__dir}
...
if ! [[ "${arg_i:?}" = "1" ]]; then
  ...
  info "Preparing Zim installer ..."
  sed "s/__version__/${__zim_ver}/g" "${__dir}"/src/file_version_info.txt.in > "${__dir}"/build/file_version_info.txt
  # set seed to a known repeatable integer value
  PYTHONHASHSEED=1
  export PYTHONHASHSEED
  pyinstaller -y src/zim.spec
